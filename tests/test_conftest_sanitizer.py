...
from tests.conftest import sanitize_cassette_file, sanitize_response_body
...
def test_sanitize_gzip_body_redacts_secret_and_host():
    body = {'base64_string': gzip_b64(json.dumps(BODY))}
...
def test_sanitize_is_noop_without_sensitive_data():
    body = {'base64_string': gzip_b64('{"x": 1}')}
...
def test_sanitize_cassette_file_scrubs_all_interactions(tmp_path):
    ...
    cassette.write_text(
        json.dumps(
            {
                'http_interactions': [
                    {'response': {'body': {'base64_string': gzip_b64(json.dumps(BODY))}}},
                    {'response': {'body': {'string': json.dumps(BODY)}}},
                ]
            }
        )
    )
    sanitize_cassette_file(cassette, REAL_HOST)
    data = json.loads(cassette.read_text())
    for interaction in data['http_interactions']:
        raw = decode(interaction['response']['body'])
